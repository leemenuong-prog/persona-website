...
- **软边揭示 UX（2026-07-07 二迭代定稿，卡片+立方体面共用）**：径向渐变 mask 羽化边缘（`#000 52% → rgba(0,0,0,.4) 76% → transparent 98%`，**禁硬边 clip-path 圆**）；进入时半径由 `attachReveal` 的 rAF 从 0 easeOutCubic 缓动扩散到目标（450ms），移动跟手、离开收拢；卡片目标半径走 `--reveal-r` 令牌（110px），立方体面 = 0.36×面宽；**立方体五个作品面悬浮显"另一张图"**（= projects.json `float.reveal` 口径，品牌面无），揭示层叠在光照遮罩之上成光斑感；3D 面用 `offsetX/Y`（浏览器逆映射局部坐标），卡片用 gBCR 差值；触屏 `(hover:none)` 隐藏 + JS `canHover` 跳过绑定；REDUCE 瞬时显隐
  - **防闪铁律（三迭代，修"还是闪"）**：单一 rAF **同帧原子写** --rv-x/--rv-y/--rv-r（此前 xy 在 pointermove、r 在另一 rAF → mask 某些帧取到不一致三元组而闪；勿再拆分写）；悬浮期给 reveal 挂 `will-change: mask-image` 隔离合成层，收拢完释放。立方体另有"悬浮停转"配合（面不在光标下扫过）
