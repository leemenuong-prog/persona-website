...
- **丝带运动体系（js/ribbon.js，2026-07-11 三迭代 strip 切片真弯曲版；单 render 循环，dt 归一保 60/120Hz 同速）**：
  ...
  - **几何**：混宽环 **R=L/2π 精确闭合**；Zoff 由环投影半宽 X_FRAC(0.32)×vw 反解（投影 scale 恒 ≤1 防合成层放大糊，全环 z≤−40 守卫）；环姿态 rotateX(**−22°**)+rotateZ(**−4°**)；**纵向锚定（四迭代定稿：环必须「套住」标题，勿再分离）=「前弧带顶边钉在标题 (1−COVER) 高度线」反解世界 y**（COVER=0.35：前弧压字前、盖字下段 ~1/3；后弧从字后绕过并露出字上方）+ **header 越界守卫**（后弧顶 < hero 顶 +8px 时把环整体下压）——凡「A 压 B 某比例」的布局**锚被压量本身**，标题换字号/文案免重调；尺寸旋钮=H_DIV(2.0)/X_FRAC(0.32)；**地面阴影 `.ribbon-floor`**（椭圆径向渐变静态一次成型、透明度恒定不呼吸，位置/尺寸由 layoutFloor() 随几何写、仅 relayout 非逐帧，z0 波点场层）；
