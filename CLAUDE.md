...
- **丝带运动体系（js/ribbon.js，2026-07-11 三迭代 strip 切片真弯曲版；单 render 循环，dt 归一保 60/120Hz 同速）**：
  ...
  - **镜像传送带路径（无状态混合）**：`pos(u)=(−r·sin u, lift, r·cos u+pull)`，u≥0=环本身、u<0（q=−u）=入场螺线：pull 恒负（远景小、「从小丝滑到大」，定标锚 SCALE_FAR；**kz 二分区间允许负**——螺线外扩本身已把远端推小）、lift 正（掠屏幕底部，定标锚 BOT_FRAC）；**⚠️ 曲率连续铁律（七迭代切痕真凶）：三个形状函数指数一律 ≥2.2**（r q^2.2 · lift/pull q^2.4；指数 <2 → 二阶导在接环点发散=曲率无穷尖峰，带子过接环点必折，加密条数治不了）+ 扭转斜坡 1.8 rad（扭转梯度是折角叠加项）——泵扫描全程可见折角 ≤3.25°、x=r·sin q 自然走「左下远景 → 掠底向右 → 右侧卷起 → 前弧右→左成环」；头部弧长 `S(t)=S0+ΔS·easeInOutSine+ω·R·t`（七迭代钟形速度曲线：起步=巡航速缓起 → 中段峰值 1.57×均速 → 终点导数 0 恰接 ω·R，两端皆柔），INTRO_DUR 6s；**sEnd=L=2πR 头尾精确合拢**（合拢缝=卡间缝 SEAM=0.025H、自然落正前）——勿改回「路径态→环态 blend」（欧拉角插值有跳变坑）；
...
- **版式**：A4 定版 `.sheet`（mm 页盒 + overflow:hidden + break-after:page），共 **24 页**（2026-07-11 V2 文案改版：Yi-Jian 审议+推演两页合一，用户裁定「太冗余」）；文案改长会触发溢出探测（屏显红描边 + console.warn），别让它默默裁切。**六作体系**：rail/INDEX 01-06，第 06 项=建筑合集（JS 由 architecture 合成，products 保持 5 项）。**索引双档**：产品封面页=完整 26mm rail；场景/线稿/建筑页=极简 rail（`.rail--mini`，主区加宽到 167mm）。
...
- **正文排满规则（2026-07-08 用户裁定）**：正文页尽量排满——内容不足半页的模块并入相邻页。落地实现：有场景页的作品，线稿块并入首场景页顶部（不单独成页）；仅无场景页的作品保留独立线稿页。共 **24 页**（3 前页 + Pears 4 / Co-work 4 / Yi-Jian 3 / Meco 4 / UABB 4 + 建筑 1 + 封底；Pears/Yi-Jian/Meco 各带「brief」文字页=线稿块+Why+核心贡献；Yi-Jian 第三页=审议+推演合页：HIW 伴文侧图（功能模块 28mm 竖长图）+三联排+Impact 节，**0.39 竖长比例图放两图行会把行高顶到 ~100mm 炸页，只能走伴文侧图或三联排**；UABB 无线稿、首场景页=Why+一文一图+核心贡献混排、尾场景页=**纯图页（scene 省 copy_ref=不渲染节头不补正文，tests 已放行）**——UABB 的 Impact 节 2026-07-11 V2 用户删除，全册与画廊都不再渲染，勿加回）。
