...
- **软边揭示 UX（径向渐变 mask 羽化边缘 `#000 52% → rgba(0,0,0,.4) → transparent`，禁硬边 clip-path 圆）——卡片与立方体两条独立路径（五迭代分家）**：
  - **画廊卡（平面 2D）`.layer-reveal`**：光标跟随聚光。`attachReveal`（index-fx.js）单帧**原子写** --rv-x/y/r（三元组必须同帧写，勿拆分——拆分会 mask 取到不一致三元组而**连续闪**）；rAF easeOutCubic 0→`--reveal-r`(110px)，移动跟手、离开收拢；常驻 `transform: translateZ(0)` 静态提层（2D 安全）。
  - **立方体面（旋转 3D）`.face-reveal`**：**居中绽放**，非光标跟随。**铁律：绝不逐面绑 pointerenter/leave**——旋转 3D 面投影重叠、命中面高频切换 → 逐面 enter/leave 让半径反复开合=闪（这是四迭代都没根治的真凶）。改为 **cube 级** hover（只绑稳定不旋转的 `.cube-vp`，全程一次 enter/一次 leave）→ 驱动**单一** `--rv-r` 写在 `#cube` 上、靠 CSS 继承给所有 `.face-reveal`（mask `at 50% 50%` 居中）；半径在 render 循环里单调缓动（0↔cubeHalf×0.92），构造上无振荡 = 不闪。**face-reveal 禁 translateZ(0)**（3D 面上会引背面剔除/合成闪）。五个作品面显 float 图（品牌面无）。
  - 触屏 `(hover:none)` 隐藏 + JS `canHover` 跳过；**切勿改回悬浮动态挂 will-change**（提层那帧漏 mask 整图闪一下）。
